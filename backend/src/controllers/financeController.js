...
class FinanceController {
  async addIncome(req, res) {
    try {
      const income = await financeService.addIncome(req.body, req.user?.id || 1);
      return res.status(201).json({ success: true, data: income, message: 'Income recorded successfully' });
    } catch (error) {
  ...
  async addExpense(req, res) {
    try {
      const expense = await financeService.addExpense(req.body, req.user?.id || 1);
      return res.status(201).json({ success: true, data: expense, message: 'Expense recorded successfully' });
    } catch (error) {
      return res.status(400).json({ success: false, message: error.message });
    }
  }
