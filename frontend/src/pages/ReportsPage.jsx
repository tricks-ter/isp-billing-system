...
import { TrendingUp, TrendingDown, Wallet, Smartphone, Building2, Calendar, Download } from 'lucide-react';
...
export default function ReportsPage() {
  ...
  const handleExportDaily = () => {
    if (!dailyData?.payments || dailyData.payments.length === 0) {
    ...
    const data = dailyData.payments.map(p => ({
    ...
    exportCSV(data, columns, `daily_collection_${selectedDate}`);
  };
  ...
  const handleExportMonthly = () => {
    ...
    const data = [
      { metric: 'Total Invoices', value: monthlyData.totalInvoices },
      { metric: 'Total Amount', value: `৳${monthlyData.totalAmount.toLocaleString()}` },
      { metric: 'Collected', value: `৳${monthlyData.totalPaid.toLocaleString()}` },
      { metric: 'Outstanding', value: `৳${monthlyData.totalDue.toLocaleString()}` },
      { metric: 'Paid Invoices', value: monthlyData.paidInvoices },
      { metric: 'Unpaid Invoices', value: monthlyData.unpaidInvoices },
    ];
    exportCSV(data, columns, `monthly_summary_${selectedMonth}`);
  };
