...
const navItems = [
  ...
  { to: '/customers', icon: Users, label: 'Customers', section: 'main' },
  ...
  { to: '/reports', icon: BarChart3, label: 'Reports', section: 'finance' },
  { to: '/users', icon: Shield, label: 'Users', section: 'system' },
  { to: '/audit-logs', icon: Activity, label: 'Audit Logs', section: 'system' },
    ...
    <div className="min-h-screen bg-background flex">
      ...
      `}>
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-1">
          ...
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider px-3 py-2 mt-4">Finance & Reports</p>
          {navItems.filter((i) => i.section === 'finance').map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              onClick={() => setSidebarOpen(false)}
